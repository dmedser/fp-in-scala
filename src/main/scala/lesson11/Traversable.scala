...
object Traversable {
  ...
  implicit val optionTraversable: Traversable[Option] = new Traversable[Option] {
    def traverse[G[_] : Applikative, A, B](option: Option[A])(f: A => G[B]): G[Option[B]] =
      option.fold(Applikative[G].pure(Option.empty[B]))(a => Applikative[G].pure(f(a)))
  }
  ...
  implicit val idTraversable: Traversable[λ[α => α]] = new Traversable[λ[α => α]] {
    def traverse[G[_] : Applikative, A, B](a: A)(f: A => G[B]): G[B] = Applikative[G].pure(f(a))
  }
...
object Tree {
  ...
  implicit val treeInorderDFSTraversable: Traversable[Tree] = new Traversable[Tree] {
    def traverse[G[_] : Applikative, A, B](tree: Tree[A])(f: A => G[B]): G[Tree[B]] = tree match {
      case Branch(value, left, right) => Applikative[G].map3(traverse(left)(f), f(value), traverse(right)(f)) { case (lb, b, rb) => Branch(b, lb, rb) }
  ...
  implicit val treePreorderDFSTraversable: Traversable[Tree] = new Traversable[Tree] {
    def traverse[G[_] : Applikative, A, B](tree: Tree[A])(f: A => G[B]): G[Tree[B]] = tree match {
      case Branch(value, left, right) => Applikative[G].map3(f(value), traverse(left)(f), traverse(right)(f)) { case (b, lb, rb) => Branch(b, lb, rb) }
      case Leaf(value) => Applikative[G].map(f(value))(Leaf(_))
    }
  }
  ...
  implicit val treePostorderDFSTraversable: Traversable[Tree] = new Traversable[Tree] {
    def traverse[G[_] : Applikative, A, B](tree: Tree[A])(f: A => G[B]): G[Tree[B]] = tree match {
      case Branch(value, left, right) => Applikative[G].map3(traverse(left)(f), traverse(right)(f), f(value)) { case (lb, rb, b) => Branch(b, lb, rb) }
      case Leaf(value) => Applikative[G].map(f(value))(Leaf(_))
    }
  }
