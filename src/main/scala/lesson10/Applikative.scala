...
trait Manoidal[F[_]] extends Funktor[F] {
  def unit: F[Unit]

  def product[A, B](fa: F[A], fb: F[B]): F[(A, B)]
}

trait Applikative[F[_]] extends Manoidal[F] {
...
object Aplikative {
  ...
  implicit def someExpApplikative[F[_] : ContrApplikative]: Applikative[λ[α ⇒ F[α] ⇒ α]] = new Applikative[λ[α ⇒ F[α] ⇒ α]] {
    ...
    def product[A, B](faa: F[A] ⇒ A, fbb: F[B] ⇒ B): F[(A, B)] ⇒ (A, B) = fab ⇒ {
      val fa = implicitly[ContrApplikative[F]].map(fab) { case(a, _) ⇒ a}
      val fb = implicitly[ContrApplikative[F]].map(fab) { case(_, b) ⇒ b}
      (faa(fa), fbb(fb))
    }
...
object ContrAplikative {
  ...
  implicit def prodContrApplikative[F[_] : ContrApplikative, G[_] : ContrApplikative]: ContrApplikative[λ[α ⇒ (F[α], G[α])]] = new ContrApplikative[λ[α ⇒ (F[α], G[α])]] {
    ...
    def lift[A, B](f: A ⇒ B): ((F[A], G[A])) ⇒ (F[B], G[B]) = { case(fa, ga) ⇒
      (implicitly[ContrApplikative[F]].map(fa)(f), implicitly[ContrApplikative[G]].map(ga)(f))
    }
  }
  ...
  implicit def compContrApplikative[F[_] : Applikative, G[_] : ContrApplikative]: ContrApplikative[λ[α ⇒ F[G[α]]]] = new ContrApplikative[λ[α ⇒ F[G[α]]]] {
    ...
    def lift[A, B](f: A ⇒ B): F[G[A]] ⇒ F[G[B]] = fga ⇒ implicitly[Applikative[F]].map(fga)(ga ⇒ implicitly[ContrApplikative[G]].map(ga)(f))
  }
  ...
  implicit def expContrApplikative[F[_] : Funktor, G[_] : ContrApplikative]: ContrApplikative[λ[α ⇒ F[α] ⇒ G[α]]] = new ContrApplikative[λ[α ⇒ F[α] ⇒ G[α]]] {
    ...
    def lift[A, B](f: A ⇒ B): (F[A] ⇒ G[A]) ⇒ F[B] ⇒ G[B] = faga ⇒ {
      fb ⇒ {
        val fa = ???
        val ga = faga(fa)
        implicitly[ContrApplikative[G]].map(ga)(f)
      }
    }
  }
}
