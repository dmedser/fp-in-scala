...
trait Applikative[F[_]] extends Funktor[F] with Manoidal[F] {
  ...
  def map2[A, B, C](fa: F[A], fb: F[B])(f: (A, B) => C): F[C] = map(product(fa, fb)) { case (a, b) => f.apply(a, b) }

  def map3[A, B, C, D](fa: F[A], fb: F[B], fc: F[C])(f: (A, B, C) => D): F[D] = {
    val fab = map(product(fa, fb))(identity[(A, B)])
    map(product(fab, fc)) { case ((a, b), c) => f.apply(a, b, c) }
  }
}

object Applikative {

  def apply[F[_] : Applikative]: Applikative[F] = implicitly
  ...
  implicit val listApplikative: Applikative[List] = new Applikative[List] {
    def lift[A, B](f: A => B): List[A] => List[B] = la => la.map(f)
  ...
  implicit def prodApplikative[F[_] : Applikative, G[_] : Applikative]: Applikative[λ[α => (F[α], G[α])]] =
    new Applikative[λ[α => (F[α], G[α])]] {
  ...
  implicit def compApplikative[F[_] : Applikative, G[_] : Applikative]: Applikative[λ[α => F[G[α]]]] =
    new Applikative[λ[α => F[G[α]]]] {
      def unit: F[G[Unit]] =
  ...
  implicit def someSumApplikative[F[_] : Applikative, M : Monoid]: Applikative[λ[α => M Either F[α]]] =
    new Applikative[λ[α => M Either F[α]]] {
  ...
  implicit def pointedApplikative[F[_] : Applikative]: Applikative[λ[α => α Either F[α]]] =
    new Applikative[λ[α => α Either F[α]]] {
  ...
  implicit def someExpApplikative[F[_] : ContrApplikative]: Applikative[λ[α => F[α] => α]] =
    new Applikative[λ[α => F[α] => α]] {
      def unit: F[Unit] => Unit = _ => ()

      def product[A, B](faa: F[A] => A, fbb: F[B] => B): F[(A, B)] => (A, B) = ???

      def lift[A, B](f: A => B): (F[A] => A) => F[B] => B = faa => { fb =>
        {
...
object ContrApplikative {
  ...
  implicit def prodContrApplikative[F[_] : ContrApplikative, G[_] : ContrApplikative]: ContrApplikative[λ[α => (F[α], G[α])]] =
    new ContrApplikative[λ[α => (F[α], G[α])]] {
  ...
  implicit def compContrApplikative[F[_] : Applikative, G[_] : ContrApplikative]: ContrApplikative[λ[α => F[G[α]]]] =
    new ContrApplikative[λ[α => F[G[α]]]] {
  ...
  implicit def expContrApplikative[F[_] : Funktor, G[_] : ContrApplikative]: ContrApplikative[λ[α => F[α] => G[α]]] =
    new ContrApplikative[λ[α => F[α] => G[α]]] {
...
object Free {
  implicit def freeApplikative[F[_]](implicit F: Funktor[F]): Applikative[λ[α => Free[F, α]]] =
    new Applikative[λ[α => Free[F, α]]] {
